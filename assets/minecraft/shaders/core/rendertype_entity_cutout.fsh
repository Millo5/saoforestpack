...
void main() {
    ...
    if (compfloat(isSkybox, 1.0)) {
		...
		float size = 270.;
		// vec4 overlay = vec4(
		// 	mod(texCoord0.x * size, 1.0),
		// 	mod(texCoord0.y * size * 1.5, 1.0),
		// 	mod(0., 1.0),
		// 	1.
		// );

		vec2 localUv = vec2(mod(texCoord0.x * size, 1.0), mod(texCoord0.y * size * 1.5, 1.0));


		float distortion = sin(localUv.y * 32.) * 0.1;
		vec4 overlay = vec4(
			distortion,
			distortion,
			distortion,
